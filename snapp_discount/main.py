...
class web_driver():
    ...
    def scrool(self,repeat=5,more_view=False):
        for iter in range(int(repeat)):
            ...
            if more_view:
                ...
                self.find_element(element='/html/body/div[1]/div/main/div[2]/div/div[2]/a/button',click=True)   # click cities
...
class get_foods():
    ...
    def set_city_page(self,city = 'اصفهان'):
        ...
        self.web.find_element(element='/html/body/div[1]/div/div/main/div/div[2]/div[2]/div[3]/div/p',click=True)   # click restaurant
    ...
    def set_area_page(self,area = 'خاقانی'):
        ...
        self.web.sendkeys(element='/html/body/div[2]/div[2]/div/section/div/div/input',value=area)             # select city
        time.sleep(0.1)
    ...
    def get_all_restaurants_name_city(self,cache=False):
        if not cache:
            none=0
            self.web.scrool(repeat=15,more_view=True)
            self.res_names=[]
    ...
    def close_set_city(self):

        try:
            self.web.find_element(element='/html/body/div[2]/div/div/div[1]/button',click=True)       # select restaurant
            return True
...
class json_cache():
    ...
    def write_restaurant_prices(self,city_name,res_name , value):
        ...
        path = os.path.join(PRICE_RES_CACHE_PATH,city_name,res_name)

        self.write_json(path,dict=value)




    def update_restaurant_prices(self,city_name,res_name,value):
        ...
        data = self.read_json(file_path=path)
